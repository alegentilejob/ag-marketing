            ...
            {sections.skills.hard.map((skill: string, index: number) => (
              <FadeIn key={index} delay={0.1 * index}>
                <li className="flex items-center gap-4 group">
                  ...
                  <span className="text-lg md:text-xl font-bold uppercase tracking-tight group-hover:text-blue-600 transition-colors">{skill}</span>
                </li>
              </FadeIn>
            ))}
          </ul>

          <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-gray-400 mt-16 mb-10 border-b border-gray-100 dark:border-gray-800 pb-4">
            {lang === 'it' ? 'Soft Skills' : 'Soft Skills'}
          </h3>
          <div className="flex flex-wrap gap-2">
            {sections.skills.soft.map((skill: string, index: number) => (
              <FadeIn key={index} delay={0.3 + 0.1 * index} className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 text-[10px] font-bold uppercase tracking-widest">
                {skill}
              </FadeIn>
            ))}
          </div>
        ...
        <div className="lg:col-span-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {sections.skills.software.map((sw: any, index: number) => (
              <FadeIn key={index} delay={0.1 * index} className="p-8 border border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 hover:border-blue-600/30 transition-all group">
                <div className="flex justify-between items-start mb-6">
                  <div className="p-3 bg-white dark:bg-black border border-gray-100 dark:border-gray-800">
                    {getIcon(sw.category)}
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{sw.category}</span>
                </div>
                <h4 className="text-xl md:text-2xl font-bold mb-2 group-hover:text-blue-600 transition-colors uppercase tracking-tighter">{sw.name}</h4>
