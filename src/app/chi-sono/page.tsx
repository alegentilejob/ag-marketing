...
import { FadeIn, RevealText, RevealImage } from '@/components/animations';
    ...
    <PageLayout>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-start">
        ...
        <div className="lg:sticky lg:top-40">
          <RevealImage delay={0.2} className="aspect-[4/5] bg-gray-50 dark:bg-gray-900 overflow-hidden border border-gray-100 dark:border-gray-800">
            <img 
          ...
          <div className="mt-10 space-y-5">
            <div className="flex items-center gap-4 text-gray-500">
              ...
              <span className="text-base font-light">{siteConfig.meta.location}</span>
            </div>
            <div className="flex items-center gap-4 text-gray-500">
              <Calendar size={18} className="text-blue-600 shrink-0" />
              <span className="text-base font-light">{siteConfig.meta.birthdate}</span>
            </div>
            <div className="flex items-center gap-4 text-gray-500">
              <Mail size={18} className="text-blue-600 shrink-0" />
              <span className="text-base font-light underline underline-offset-4 break-all">{siteConfig.contact.email}</span>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {siteConfig.meta.languages.map((l: any) => (
                <div key={l.name} className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-400 font-medium uppercase tracking-widest">{l.level}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          <header className="mb-14">
            <RevealText>
              <h1 className="text-4xl md:text-7xl font-bold tracking-tighter mb-8 uppercase leading-tight">
          ...
          <section className="mb-16">
            ...
            <div className="space-y-10">
              {sections.about.education.map((edu: any, index: number) => (
                <FadeIn key={index} delay={0.1 * index} className="relative pl-8 border-l-2 border-gray-100 dark:border-gray-800">
                  <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-blue-600" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block mb-2">{edu.period}</span>
                  <h4 className="text-lg font-bold mb-2 uppercase tracking-tight">{edu.institution}</h4>
